...
const AdminDashboard = () => {
    ...
    const filteredUsers = useMemo(() => {
        ...
        if (statusFilter !== 'All') {
            const isPremium = statusFilter === 'Premium';
            result = result.filter(u => u.is_premium === isPremium);
        }
        return result;
        ...
        <div className="flex h-screen bg-slate-50 dark:bg-[#020617] -mt-20 pt-20 overflow-hidden relative">
            ...
            <main className="flex-1 h-full overflow-y-auto w-full p-4 lg:p-8 scroll-smooth">
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                    ...
                    {/* Premium Users KPI */}
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass-panel p-6 relative overflow-hidden group">
                        <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:scale-110 transition-transform"><Crown className="w-24 h-24" /></div>
                        <div className="flex items-center gap-4 mb-4">
                            <div className="w-12 h-12 rounded-xl bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 flex items-center justify-center"><Crown className="w-6 h-6" /></div>
                            <h3 className="text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider text-xs">Premium Subs</h3>
                        </div>
                        <h2 className="text-4xl font-black text-slate-900 dark:text-white">{usersList.filter(u => u.is_premium).length}</h2>
                        <p className="text-sm text-emerald-500 font-medium mt-2 flex items-center gap-1">Conversion: {usersList.length ? Math.round((usersList.filter(u => u.is_premium).length / usersList.length) * 100) : 0}%</p>
                    </motion.div>
                            ...
                            <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
                                ...
                                <div className="relative">
                                    <select 
                                        ...
                                        className="appearance-none pl-4 pr-10 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-primary/50 cursor-pointer"
                                    >
                                ...
                                <tbody className="divide-y divide-slate-50 dark:divide-slate-800/50">
                                    {currentUsers.map((u) => (
                                        <tr key={u.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                                            ...
                                            <td className="px-6 py-4">
                                                {u.is_premium
                                                    ? <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 text-xs font-bold border border-amber-200/50 dark:border-amber-700/50 shadow-sm"><Crown className="w-3 h-3" /> PRO</span>
                                                    : <span className="inline-flex items-center gap-1px-3 py-1 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 text-xs font-semibold px-3 py-1">Basic</span>
                                                }
                                            </td>
